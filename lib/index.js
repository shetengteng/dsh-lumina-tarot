...
// src/interpret.ts
var FOLLOWUP_GUARD = "__dshLuminaFollowupGuard";
var DEDUPE_MS = 2e3;
function cardLine(card) {
  const mark = card.reversed ? "\uFF08\u9006\uFF09" : "";
  return `- ${card.positionName} \xB7 ${card.name}${mark}\uFF1A${card.meaning}`;
}
...
function claimFollowup(readingId, commandId) {
...
async function followupInterpret(agent, reading, commandId) {
...
function registerLuminaCommands(ctx, state) {
  ctx.commands.register({
    ...
    recordInput: false,
    handler: async ({ commandId, rawInput, agent, signal }) => {
